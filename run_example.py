...
from generated_tempdir_2020_12_15_095719.generated_part000000 import match_root
from sympy import *
...
for expr, expected in expr_result:
    ...
    print(expr)
    print("CANDIDATES")
    for repl, subst, constraints in ret:
        print("Match candidate:")
        print("Candidate:", repl)
        print("Subst:", subst)
        print("Constraints:")
        c = True
        for cons in constraints:
            print("    ", cons)
            c2 = cons(**{k: v for k, v in subst.items() if k in cons.__code__.co_varnames})
            print("    ", c2)
            if not c2:
                c = False
                break
        print("Constraint satisifed:", c)
        if c:
            r = repl(**subst)
            print("    Result:", r)
        print("--")
